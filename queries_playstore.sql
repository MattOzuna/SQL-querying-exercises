...
--This works but correct answer is 
...
SELECT app_name, MAX(price), rating FROM analytics WHERE rating < 3 GROUP BY app_name,rating ORDER BY MAX(price) DESC LIMIT 1;
--This works but correct answer is 
...
SELECT app_name FROM analytics WHERE min_installs <= 50 AND rating > 0 ORDER BY rating DESC, app_name;
--This works but not correct
SELECT * FROM analytics 
...
WHERE min_installs >= 100000 AND min_installs/reviews = (SELECT MAX(min_installs/reviews) FROM analytics);
